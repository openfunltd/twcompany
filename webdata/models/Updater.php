...
class Updater
{
    ...
    public static function parseFile($content)
    {
        ...
        foreach ($base_table_dom->getElementsByTagName('tr') as $tr_dom) {
            ...
            if (in_array($column, array('統一編號', '公司名稱', '資本總額(元)', '實收資本額(元)', '代表人姓名', '公司所在地', '登記機關', '股權狀況', '公司屬性', '停業核准(備)機關'))) {
                ...
                $info->{'舊營業項目資料'} = 'https://gcis.nat.gov.tw' . $tr_dom->getElementsByTagName('td')->item(1)->getElementsByTagName('a')->item(0)->getAttribute('href');
            } else {
                $key_dom = $tr_dom->getElementsByTagName('td')->item(1);
                $value_dom = $tr_dom->getElementsByTagName('td')->item(2);
                echo '[TODO1]' . trim($key_dom->nodeValue) . ' ' . $value_dom->nodeValue . "\n";
    ...
    public static function updateBranch($id, $options = array())
    {
        ...
        $url = 'https://serv.gcis.nat.gov.tw/pub/cmpy/branInfoAction.do?method=detail&brBanNo=' . $id;
        // 一秒只更新一個檔案
        while (!is_null(self::$_last_fetch) and (microtime(true) - self::$_last_fetch) < 0.5) {
            usleep(1000);
        }
    ...
    public static function update($id, $options = array())
    {
        ...
        $url = 'https://serv.gcis.nat.gov.tw/pub/cmpy/cmpyInfoAction.do?method=detail&banNo=' . $id;
        // 一秒只更新一個檔案
        while (!is_null(self::$_last_fetch) and (microtime(true) - self::$_last_fetch) < 0.5) {
            usleep(1000);
        }
    ...
    {
        $tmpfile = tempnam('', '');
        $url = 'https://gcis.nat.gov.tw/pub/cmpy/branInfoListAction.do?method=query&banNo=' . str_pad(intval($id), 8, '0', STR_PAD_LEFT) . '&from=';
        $curl = curl_init();
        if (getenv('PROXY_URL')) {
            curl_setopt($curl, CURLOPT_PROXY, getenv('PROXY_URL'));
        }
        ...
        for ($i = 2; $i <= $total; $i ++) {
            sleep(1);
            curl_setopt($curl, CURLOPT_COOKIEFILE, $tmpfile);
            curl_setopt($curl, CURLOPT_URL, 'https://serv.gcis.nat.gov.tw/pub/cmpy/branInfoListAction.do');
            curl_setopt($curl, CURLOPT_REFERER, 'https://serv.gcis.nat.gov.tw/pub/cmpy/branInfoListAction.do');
    ...
    public static function searchByKeyword($word)
    {
        ...
        $tmpfile = tempnam('', '');
        $url = 'https://serv.gcis.nat.gov.tw/pub/cmpy/cmpyInfoListAction.do';
        $curl = curl_init();
        if (getenv('PROXY_URL')) {
            curl_setopt($curl, CURLOPT_PROXY, getenv('PROXY_URL'));
        }
