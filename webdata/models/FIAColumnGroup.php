...
class FIAColumnGroup extends Pix_Table
{
    public function init()
    {
        $this->_name = 'fia_columngroup';
        $this->_primary = 'id';
    ...
    public static function getColumnName($id)
    {
        if (is_null(self::$_column_names)) {
            self::$_column_names = FIAColumnGroup::search(1)->toArray('name');
        }
    ...
    public static function getColumnId($name)
    {
        ...
        if (is_null(self::$_column_groups)) {
            ...
            foreach (FIAColumnGroup::search(1) as $group) {
                self::$_column_groups[$group->name] = $group->id;
            }
        }

        if (!array_key_exists($name, self::$_column_groups)) {
            self::$_column_groups[$name] = FIAColumnGroup::insert(array('name' => $name))->id;
        }
