...
for ($time = $start; $time < mktime(0, 0, 0, date('m'), 1) - 86400; $time = strtotime('next month', $time)) {
    ...
    foreach (FIAUnitChangeLog::search(sprintf("updated_at >= %d AND updated_at < %d", $time, strtotime('next month', $time)))->volumemode(10000) as $unit) {
        fputs($fp, json_encode(array(
            $unit->id,
            intval($unit->updated_at),
            FIAColumnGroup::getColumnName($unit->column_id),
    ...
    S3Lib::putFile($tmpname, 's3://ronnywang-twcompany/tax-history/' . $file_name);
